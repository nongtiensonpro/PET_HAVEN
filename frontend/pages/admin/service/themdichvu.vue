      ...
      <div class="card-body">
        ...
        <div class="mb-3">
          ...
          <div class="d-flex align-items-center">
            <input type="file" ref="fileInput" @change="handleFileUpload" accept="image/*" class="form-control"
              style="max-width: 300px;">
            <button @click="fileInput?.click()" class="btn btn-outline-secondary ms-2">{{ t('select') }} file</button>
          </div>
          <img v-if="newService.anh" :src="newService.anh" :alt="t('image')" class="img-fluid mt-2"
        ...
        <div v-for="(tuyChon, index) in newService.tuyChonDichVus" :key="tuyChon.id" class="card mt-3">
          <div class="card-body">
            ...
            <textarea v-model="tuyChon.moTa" class="form-control mb-2" :placeholder="t('description')"
            ...
            <ul class="list-group">
              <li v-for="(canNang, canNangIndex) in tuyChon.tuyChonCanNangs" :key="canNang.id" class="list-group-item">
                <div class="row align-items-center">
                  <div class="col">
                    <input v-model="canNang.canNangMin" type="number" class="form-control"
                      :placeholder="t('weight') + 'min'">
                  </div>
                  <div class="col">
                    <input v-model="canNang.canNangMax" type="number" class="form-control"
                  ...
                  <div class="col-auto">
                    <button @click="removeCanNang(tuyChon, canNangIndex)" class="btn btn-danger btn-sm">
                      {{ t('delete') }}
                    </button>
                  </div>
                </div>
            ...
            <button @click="addCanNang(tuyChon)" class="btn btn-secondary mt-2">{{ t('addTimeSlot') }}
              {{ t('weight') }}
            </button>
          </div>
        </div>
        <button @click="addTuyChon" class="btn btn-secondary mt-3">{{ t('addTimeSlot') }} {{ t('serviceOptions') }}
        </button>
        <div class="mt-4">
