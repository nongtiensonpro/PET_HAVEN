...
<template>
  <div class="card">
    ...
    <div v-if="selectedService" class="card-body">
      ...
      <button @click="doiDichVu" class="btn btn-primary mt-3" :disabled="!canConfirm">
        {{ t('confirm') }}
      </button>
    </div>
  </div>
...
import DichVu, {TuyChonCanNang, TuyChonDichVu} from "~/models/DichVu";
...
const id = route.params.id;  // Lấy tham số id từ URL
const ghichu = ref('');
const doiDichVu = async () => {
  ...
  try {
    ...
    const response = await fetch("http://localhost:8080/api/hoa-don/doi-dich-vu", {
      method: "PUT",  // Hoặc "PUT" nếu bạn muốn cập nhật
      headers: {
        "Content-Type": "application/json",  // Chỉ định kiểu nội dung là JSON
        "Authorization": `Bearer ${token}`,
      },
      body: JSON.stringify(data),  // Chuyển đối tượng dữ liệu thành JSON
    });
    ...
    if (response.ok) {
      ...
      console.error("Có lỗi khi gửi yêu cầu:", response.statusText);
    }
  } catch (error) {
